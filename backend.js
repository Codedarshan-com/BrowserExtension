...
function getApiKey() {
  ...
  currentKeyIndex = (currentKeyIndex + 1) % keys.length; // Rotate the key index
  return key;
}
...
app.post("/sendinput", async (req, res) => {
  ...
  try {
    console.log(
      `Received request with user answer: ${useranswer} and URL: ${url}`
    );

    // Initialize the OpenAI client with a rotated API key
    ...
    const aiResponse = await client.chat.completions.create({
      ...
      messages: [
        ...
        2. **Language and Formatting**:
           ...
           - Clearly format hints using the structure: ** X:** followed by the suggestion, and leave two blank lines between hints for readability.
        ...
        6. **Example Hint Structure**:
           - Start hints with ** X:** and focus on one concept per hint.
           - Examples:
             - ** 1:** "Consider the edge cases where the input array is empty or contains a single element. How might your current implementation handle these?"
             - ** 2:** "Your current time complexity is O(n^2). Can you think of a way to optimize it to O(n log n) by using a sorting algorithm?"
        ...
        5. Separate each hint with " X:" at the start and end with two line breaks for spacing.
        `,
        },
      ],
    });
